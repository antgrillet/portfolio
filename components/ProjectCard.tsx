...
function getGradientForTitle(title: string): string {
	...
	// Générer un index basé sur le titre
...
export function ProjectCard({
				...
				{imageUrl ? (
					...
					<div className="absolute inset-0 flex items-center justify-center">
						<div className="text-center">
							<div className="text-6xl font-bold text-white/90 mb-2">{initials}</div>
							<div className="text-sm text-white/70 font-medium px-4">{title}</div>
						</div>
					</div>
				)}
	...
	return (
		<motion.div
			...
			viewport={{ once: true }}
			className="group relative overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:shadow-2xl hover:shadow-blue-500/10 dark:hover:shadow-blue-500/5 transition-all duration-300 hover:-translate-y-1"
		>
			{liveUrl ? (
				<a
					href={liveUrl}
